...
include { BIOPHI_SAPIENS } from '/home/ubuntu/oliver/nf-core-antibody-pipeline/modules/local/biophi-oasis/biophi/tests/../main.nf'
...
workflow {
    ...
    input = []
                ...
                input[0] = [
                    [ id:'test_antibody' ],
                    file("/home/ubuntu/oliver/nf-core-antibody-pipeline/modules/local/biophi-oasis/biophi/tests/test_input.fasta")
                ]
                
    //----

    //run process
    BIOPHI_SAPIENS.run(input.toArray())

    if (BIOPHI_SAPIENS.output){

        // consumes all named output channels and stores items in a json file
        BIOPHI_SAPIENS.out.getNames().each { name ->
            serializeChannel(name, BIOPHI_SAPIENS.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  

        // consumes all unnamed output channels and stores items in a json file
        def array = BIOPHI_SAPIENS.out as List<Object>
        def i = 0
        array.each { output ->
